...
export default function Home() {
  ...
  return (
    <>
      <header className="h-screen max-h-[800px] xl:max-h-none !pt-0 p-4 md:p-7 lg:p-10 w-full flex flex-col justify-end items-start bg-[#B3B3B3] bg-contain bg-bottom bg-no-repeat mb-16 md:mb-[104px] bg-[url('/assets/images/hero-image.png')] relative" id="#">
        <HeaderTextMarquee />
        <p className={`${geist.className} absolute top-1/2 -translate-y-1/2 text-white w-[120px] md:w-[180px] lg:w-[450px] text-end right-[16px] md:right-[28px] lg:right-10 text-[20px]/[26px] md:text-[28px]/[36px] 2xl:text-[50px] font-normal tracking-[0px]`}>
          Transform your bold ideas into flawless designs.
        </p>
        <div className="absolute flex flex-col justify-center items-start left-[16px] md:left-[28px] lg:left-10 top-1/2 -translate-y-1/2">
              ...
              designs.map((design, index) => (
                <div className="flex flex-col gap-3 group !cursor-pointer" key={index}>
                  <div className="relative overflow-hidden h-[300px] sm:h-[400px] 2xl:h-[500px] w-full bg-contain bg-[#C8C8C8]">
                    <Image src={design.image} alt="Designs for clients who have trusted me with their project design" fill className={`${design.status !== 'live' ? 'hover:scale-0' : ''}lg:hover:scale-125 transition-all ease-linear duration-300`} />
                  </div>
                    ...
                    </p>
                    {
                      ...
                      <a href={design.url}>
                        <button className="lg:opacity-0 lg:group-hover:opacity-100 transition-all ease-linear duration-300 flex items-center justify-center gap-2">
                          <p className={`${geist.className} text-[16px]/[16px] md:text-[20px]/[20px] 2xl:text-[1vw] tracking-[-0.64px] md:tracking-[0.8px] font-medium text-black capitalize`}>
                            Visit Site
                          </p>
                          <span>
